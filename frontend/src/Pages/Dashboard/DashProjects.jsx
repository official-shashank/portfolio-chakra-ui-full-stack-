...
const DashProjects = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editIndex !== null) {
        await axios.put(`http://localhost:5000/api/v1/projects/projects/${projects[editIndex]._id}`, form);
      } else {
        await axios.post("http://localhost:5000/api/v1/projects/projects", form);
      }
      fetchProjects();
      resetForm();
      onClose();
  ...
  const handleDelete = async (index) => {
    try {
      await axios.delete(`http://localhost:5000/api/v1/projects/projects/${projects[index]._id}`);
      fetchProjects();
    } catch (error) {
    ...
    <Navbar>
      <Box p={5}>
        ...
        <Modal isOpen={isOpen} onClose={handleModalClose}>
          <ModalOverlay />
          <ModalContent>
            <ModalHeader>{editIndex !== null ? "Edit Project" : "Add Project"}</ModalHeader>
            <ModalCloseButton />
                ...
                <VStack spacing={4}>
                  ...
                  <FormControl isRequired>
                    ...
                    <Select
                      isMulti
                      name="teamMembers"
                      value={form.teamMembers.map(member => ({ label: member, value: member }))}
                  ...
                  <FormControl isRequired>
                    ...
                    <Select
                      isMulti
                      name="technologyUsed"
                      value={form.technologyUsed.map(tech => ({ label: tech, value: tech }))}
                      onChange={handleSelectChange}
                      options={techOptions}
                  ...
                  <FormControl>
                    ...
                    <Input name="livePreviewLink" value={form.livePreviewLink} onChange={handleChange} />
                  </FormControl>
                  <FormControl isRequired>
                ...
                <Tr key={index}>
                  ...
                  <Td>
                    {project.tags.map((tag, i) => (
                      <Badge key={i} colorScheme="purple" mr={1}>{tag}</Badge>
                    ))}
                  </Td>
                  <Td>
                    {project.technologyUsed.map((tech, i) => (
                      <Badge key={i} colorScheme="blue" mr={1}>{tech}</Badge>
                    ))}
                  </Td>
                  <Td><a href={project.livePreviewLink} target="_blank" rel="noopener noreferrer">Live Preview</a></Td>
                  <Td><a href={project.githubLink} target="_blank" rel="noopener noreferrer">GitHub</a></Td>
                  <Td>
                    {project.imageUrls.map((url, i) => (
                      <Tag key={i} mr={1} mb={1}>{url}</Tag>
                    ))}
                  </Td>
                  <Td>
                    <IconButton
